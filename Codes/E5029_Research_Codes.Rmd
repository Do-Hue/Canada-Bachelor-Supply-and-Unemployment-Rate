...
**Data availability:** This analysis uses Statistics Canada's Labour Force Survey Public Use Microdata File (PUMF). The raw microdata is too large to be hosted on GitHub.

You can download it from the following Dropbox link:
[INSERT DROPBOX LINK HERE]

After downloading, place the file in the following location relative to the project root before running the analysis:

`data/EVERMD.csv`

**Note:** Statistics Canada Labour Force Survey (LFS) microdata is subject to StatCan's licensing terms. Please review the applicable license before redistributing or using this file outside of this project.

# Setup and Data Loading
We first load required packages and define file paths. `DATA_PATH` and `OUTPUT_DIR` are relative to the project root. Make sure the LFS CSV is placed at `data/EVERMD.csv` before running this analysis.
...
# The CSV file should be placed at: data/EVERMD.csv
